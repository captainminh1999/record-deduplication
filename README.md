...
In addition, the similarity step now writes **`data/outputs/match_data.csv`**. This companion file lists the cleaned company name, domain, phone, address, state and country code for both records in each pair (`record_id_1` and `record_id_2`). It can help you verify how the matched fields compare alongside the numeric similarity scores.
