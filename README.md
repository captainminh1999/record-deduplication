...
* `run_history.log` – (optional) a log file appending the run details (stage name, timestamp, number of rows processed, and duration) if logging was enabled. By default, the script logs each run automatically.
...
**Output:** The result of this step is a set of candidate record pairs to compare. In this minimal implementation, the candidate pairs may be kept in memory or saved as an index structure. (If this were a large-scale application, you might save the index or candidate list to a file for inspection or reuse. In our pipeline, subsequent steps will reuse the blocking logic to fetch candidates as needed.) There isn’t a user-facing output file from blocking in the `data/outputs` directory – it’s an **intermediate step** feeding into similarity computation.
...
**Output:** After running, you should get a file **`data/outputs/features.csv`** containing a table of candidate pairs with their computed similarity features (each row corresponds to a pair of records, with columns for each feature and identifiers for the records compared). This features table is the input to the machine learning model in the next stage. Each row might include columns such as `cmp_name` (a similarity score for company name), `cmp_address`, `cmp_domain`, `cmp_phone_exact` (perhaps 0/1 for exact match), etc., as well as the IDs of the two records being compared. Inspecting `features.csv` can be useful for debugging – for example, to see if the features for a known duplicate pair are indeed high (close to 1) as expected.
